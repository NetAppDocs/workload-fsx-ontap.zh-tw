...
summary: 在 NetApp Workload Factory 中探索並複寫內部部署的 ONTAP 資料，以便將其用作雲端備份。 
---
...
[role="lead"]
在 NetApp Workload Factory 中探索並複寫內部部署的 ONTAP 資料，以便將其用作雲端備份。
...
* 要與探索到的內部部署叢集建立關聯的已連線連結。如果您沒有連結，您需要link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["建立一個"^]。
* 具有必要權限的 ONTAP 使用者認證。
...
.. *ONTAP credentials*：請提供以下資訊：
+
*** GovCloud 帳戶：使用 ONTAP 憑證輸入 ARN。
*** 所有其他帳戶：輸入內部部署 ONTAP 叢集的 ONTAP 憑證。確保使用者擁有所需的權限。
...
* 您用於複寫關係的內部部署 ONTAP 叢集和 FSx for ONTAP 檔案系統都必須具有關聯的連結。link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["了解如何關聯現有連結或建立並關聯新連結"^]。連結關聯後，請返回此作業。
