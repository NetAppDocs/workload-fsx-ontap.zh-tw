...
在 BlueXP  工作負載工廠中探索內部部署的 ONTAP 叢集，以便將資料複寫到 Amazon FSX for NetApp ONTAP 檔案系統。
...
+
*** * 自動 * ：使用 ONTAP 使用者介面的工作負載原廠 FSX 建立磁碟區時，預設的分層原則。將所有冷資料分層，包括在特定時間段內，將使用者資料和快照傳送至容量集區儲存層。
