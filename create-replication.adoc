...
為遷移使用案例建立複寫關係後，您必須切換複寫，才能將儲存 VM 資料和組態設定完成遷移至目標 FSx for ONTAP 檔案系統。切換複寫會將資料和儲存 VM 組態設定從來源檔案系統永久遷移至目標 FSx for ONTAP 檔案系統。在切換期間，資料會進行最後一次複寫。當切換開始時,來源儲存 VM 會離線。
...
* link:initialize-replication.html["初始化複寫"] 用於建立新的複寫關係。
