...
* *Storage VM 組態設定*：轉換作業不會將所有 Storage VM 組態設定移轉至目標 Storage VM 。例如、 iSCSI 、廣播網域 MTU 和 DNS 設定不會移轉。轉換後、請在目標 Storage VM 上手動更新這些設定。如需更多詳細資料、請參閱<<切換後更新組態設定,切換後更新組態設定>>一節。
