...
summary: 使用NetApp Workload Factory，您可以建立 FSx for ONTAP檔案系統來新增和管理磁碟區和其他資料服務。 
---
...
* 具有建立和刪除檔案系統權限的憑證，用於建立ONTAP檔案系統的 FSx。link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["瞭解如何授予 AWS 帳戶權限"^]。
...
. 在「備份與維護」下、提供下列項目：
+
.. *Volume backups*：預設啟用每日自動備份。可依需要停用。
+
