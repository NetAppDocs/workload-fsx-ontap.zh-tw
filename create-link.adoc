...
* 手動：使用 Codebox 中的 CloudFormation 或 Terraform 建立手動註冊的連結。程式碼會保留，以便您有更多時間完成操作。這在與安全和 DevOps 等不同團隊合作時很有用，因為他們可能需要先授予完成連結建立所需的權限。
...
.. *連結權限*：選擇以下連結權限選項之一：
+
*** *自動*：選擇此選項，以便 AWS CloudFormation 或 Terraform 程式碼自動建立 Lambda 權限原則和執行角色。
...
.手動建立
--
使用此選項，您可以提取連結的 ARN 並在此處報告。工作負載工廠會手動為您登錄連結。您可以使用 Codebox 中的兩個基礎架構即程式碼 (IaC) 工具建立連結：CloudFormation 或 Terraform。
