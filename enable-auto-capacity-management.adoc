...
summary: 啟用此功能後，Workload Factory 可以根據容量需求隨時間的變化自動向 FSx for ONTAP檔案系統新增增量儲存。 
---
...
[role="lead"]
啟用自動容量管理後， NetApp Workload Factory 可以根據容量需求隨時間的變化自動在 FSx for ONTAP檔案系統中新增增量儲存。此外，啟用此功能無需手動監控容量，並可降低影響應用程式資料讀寫的風險。
...
* 您必須擁有 Workload Factory 中具有讀取/寫入權限的憑證才能完成此任務。
