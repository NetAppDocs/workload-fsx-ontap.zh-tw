...
.開始之前
為了讓工作負載工廠在 AWS 帳戶中偵測適用於 Windows 檔案伺服器儲存環境的 Amazon Elastic Block Store （ EBS ）， Elastic File System （ EFS ）和 FSX ，請確定您link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["授與 _ 讀取 _ 權限"^]在 AWS 帳戶中。
